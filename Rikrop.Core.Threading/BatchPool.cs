...
namespace Rikrop.Core.Threading
{
    public class BatchPool<T> : IDisposable
    {
        ...
        private readonly Action<IReadOnlyCollection<T>> _flushAction;
        ...
        public BatchPool(Action<IReadOnlyCollection<T>> flushAction)
            : this(flushAction, DefaultFlushThreashold, TimeSpan.FromMilliseconds(DefaultFlushTimeout))
        {
        }

        public BatchPool(Action<IReadOnlyCollection<T>> flushAction, int flushThreashold)
            : this(flushAction, flushThreashold, TimeSpan.FromMilliseconds(DefaultFlushTimeout))
        {
        }

        public BatchPool(Action<IReadOnlyCollection<T>> flushAction, int flushThreashold, TimeSpan flushTimeout)
        {
